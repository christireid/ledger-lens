...
**2026-07-30 · CSP nonce requires dynamic rendering — everywhere.** The first M9 CSP pass set the nonce policy only on the response; with 'strict-dynamic' ignoring 'self', every Next script was blocked and the app silently degraded to SSR-only (caught by the media-capture run, not by the header checks; the §21.12 spec now asserts zero CSP console violations to close that hole). Correct wiring per §21.4: middleware puts nonce + policy on the request headers, the root layout reads headers() so every HTML route renders dynamically, and next-themes receives the nonce as a prop. The marketing page gives up SSG because §21.12 test-enforces no unsafe-inline in script-src on "/" as well — LCP stays ~0.5s (§20.8 evidence). Affected: 21.4, 21.12, 20.10.

**2026-07-30 · Sentry as thin DSN reporter.** §24.4 names the Sentry SDK; @sentry/nextjs's client bundle would consume most §20.3 headroom and is inert without a DSN (none configured — operator gap). Implemented the §24.4 contract as a seam: beforeSend-style deny-by-default scrub (unit-tested), release + digest/requestId tagging, fire-and-forget store-API delivery when SENTRY_DSN is set. Swapping in the SDK is contained to src/server/obs/sentry.ts. Affected: 24.4, 20.3.

**2026-07-30 · aria-expanded off ledger rows.** aria-expanded is invalid on role=row outside a treegrid (axe aria-conditional-attr, 50 nodes once hydration was fixed). Removed in favor of the revealed detail row conveying state; a treegrid conversion was rejected as heavier than §19 requires. Affected: 19, 05.5.
